...
const tableHead = ['Book Title', 'Book Availability', 'User', 'User Email', 'Status', 'Request Date', 'Return Date', 'Action']
...
export default function RequestManagement({ }) {
    ...
    const acceptDenyRequest = (request, action) => {
        console.log(request)
        if (!request.dueDate && action === 'approved') {
            handleOpen(request);
        } else {
            // Update user
    ...
    const updateRequest = (id, reqData) => {
        RequestService.updateRequest(id, reqData).then((data) => {
            setRequests(sort(requests.map(request => request._id === data._id ? { ...request, dueDate: data.dueDate, status: data.status } : request)));
        }).catch((err) => {
            console.log(err);
            if (err.response.data.statusCode === 403) {
                navigate('/signin');
    ...
    const deleteRequest = (id) => {
        RequestService.deleteRequest(id).then((data) => {
            setRequests(sort(requests.filter(req => req._id !== id)));
        }).catch((err) => {
            console.log(err);
            if (err.response.data.statusCode === 403) {
                navigate('/signin');
                        ...
                        <TableBody >
                            {requests.map((request, index) => (
                                <TableRow key={index}>
                                    ...
                                    <TableCell >{request?.book.available ? 'YES' : 'NO'}</TableCell>
                                    ...
                                    <TableCell >{
                                        request.dueDate ?
                                            <div>
                                                {request.dueDate}
                                                <IconButton
                                                    disabled={['approved', 'withdrawn', 'denied', 'returned'].includes(request.status)}
                                                    onClick={() => {
                                    ...
                                    <TableCell >
                                        <div style={{ display: 'flex', alignItems: 'center' }}>
                                            <Button
                                                size='small'
                                                disabled={['approved', 'withdrawn', 'denied', 'returned'].includes(request.status)}
                                                onClick={() => acceptDenyRequest(request, 'approved')}>
                                                Approve
                                            </Button>
                                            <Button
                                                size='small'
                                                disabled={['approved', 'withdrawn', 'denied', 'returned'].includes(request.status)}
                                                onClick={() => acceptDenyRequest(request, 'denied')}>
                                                Deny
                                            </Button>
                                            <IconButton
                                                size='small'
